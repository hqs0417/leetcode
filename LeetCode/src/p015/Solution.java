...
public class Solution {
	...
	public List<List<Integer>> threeSum(int[] nums) {
		...
		for(int i = 0; i < size - 2;  ++i) {//
			int j = i + 1;//左侧位置
			if(i > 0 && nums[i]  == nums[i-1]) continue;//相等则跳过
	...
	public List<List<Integer>> threeSum1(int[] nums) {
		...
		for(int i =0; i < n - 2; i++) {
			if(i != 0 && nums[i] == nums[i-1]) continue;//当期处理的值，与上一个元素相等，则跳过，否则会产生重复值
			...
			while(left < right) {
				if(nums[left] + nums[right] < remain) {
					...
					right--;
					result.add(triple);
					while(left < right && nums[left] == nums[left - 1] && nums[right] == nums[right + 1]) left
					++;
				}
			}
